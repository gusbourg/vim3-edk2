  ...
  )
{
  ...
  for (Index = 0; Index < Xhc->EventRing.TrbNumber; Index++) {
    ...
    switch (EvtTrb->Completecode) {
      ...
      //
      // VIM3: an endpoint that reports one of these is left in a state the
      // controller will keep rejecting - xHCI 4.8.3 requires software to reset
      // it.  Report them as a transaction error, which is the result XhcTransfer
      // acts on (Reset Endpoint + Set TR Dequeue via XhcRecoverHaltedEndpoint);
      // upstream's EFI_USB_ERR_TIMEOUT is silently dropped by that test and the
      // endpoint stays wedged for the rest of the boot.  Linux performs the
      // equivalent recovery from xhci_handle_halted_endpoint().
      //
      case TRB_COMPLETION_CONTEXT_STATE_ERROR:
      case TRB_COMPLETION_TRB_ERROR:
      case TRB_COMPLETION_SPLIT_TRANSACTION_ERROR:
        CheckedUrb->Result  |= EDKII_USB_ERR_TRANSACTION;
        CheckedUrb->Finished = TRUE;
        DEBUG ((
          DEBUG_ERROR,
          "XhcCheckUrbResult: recoverable endpoint error, Completecode = 0x%x\n",
          EvtTrb->Completecode
          ));
        goto EXIT;
  ...
  )
{
      ...
      ((PortState->PortStatus & USB_PORT_STAT_CONNECTION) != 0))
  {
    ...
    do {
      //
      // VIM3: on a retry the previous attempt failed with a transaction error
      // because the device (several hubs deep) was not ready.  Give it time to
      // settle before re-attempting Enable Slot / Address Device.
      //
      if (Status == EFI_DEVICE_ERROR) {
        gBS->Stall (XHC_DEVICE_SLOT_RETRY_STALL);
      }
  ...
  )
{
  ...
  //
  // VIM3: the loop above established, from the cycle bits alone, which entries
  // the controller has handed to software.  Order those reads before the event
  // bodies the caller is about to consume, or a body can be read from cache
  // ahead of the cycle bit that publishes it.  Linux has the same rmb() in
  // xhci_handle_event().
  //
  MemoryFence ();
...
{
  //
  // VIM3: publish the TRBs before the doorbell that tells the controller to
  // read them.  This platform registers the xHC as non-coherent, so the rings
  // are mapped Normal-NC while the doorbell is Device memory; ARMv8 does not
  // order those two against each other without an explicit barrier, so the
  // controller can otherwise observe a doorbell for a TRB whose cycle bit is
  // not visible yet and consume a stale entry.  Linux carries a wmb() at every
  // equivalent point.  MemoryFence() is "dmb sy" on AArch64.
  //
  MemoryFence ();
