...
//
// VIM3: the Amlogic DWC3 intermittently returns a USB Transaction Error for
// Address Device when the target sits several hubs down and is not yet ready
// after its reset.  Upstream's single retry loses that race at RELEASE speed
// and the deepest device never enumerates; an interrupt-driven OS always wins
// it.  Retry with a settle delay between attempts (xHCI 4.6.5 sanctions the
// Disable Slot / Enable Slot recovery this loop performs).
//
#define XHC_INIT_DEVICE_SLOT_RETRIES  8
#define XHC_DEVICE_SLOT_RETRY_STALL   (50 * 1000)
...
//
// VIM3: xHCI 1.1 table 6-90.  Upstream leaves these undefined, so they land in
// the switch default and are reported as EFI_USB_ERR_TIMEOUT - a result that is
// deliberately excluded from the halted-endpoint recovery test in XhcTransfer.
// The endpoint is therefore never reset and every later doorbell on it fails
// the same way, which is how one error turns into a dead endpoint.
//
#define TRB_COMPLETION_RESOURCE_ERROR          7
#define TRB_COMPLETION_BANDWIDTH_ERROR         8
#define TRB_COMPLETION_NO_SLOTS_ERROR          9
#define TRB_COMPLETION_SLOT_NOT_ENABLED_ERROR  11
#define TRB_COMPLETION_EP_NOT_ENABLED_ERROR    12
